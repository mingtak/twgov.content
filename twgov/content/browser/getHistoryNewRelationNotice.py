...
class GetHistoryNewRelationNotice(BrowserView):
    def __call__(self):
        logger = logging.getLogger("getHistoryNewRelationNotice.GetHistoryNewRelationNotice")
        portal = api.portal.get()
        catalog = api.portal.get_tool(name='portal_catalog')
        #取得公告BDM網址
        with open('/home/plone/zzzzz') as f:
            getBdmUrl = ' "%s" ' % f.readline()
        dateString = getBdmUrl.split('ds=')[1].split('&')[0]
        ...
        api.portal.send_email(recipient=LOG_MAIL_RECIPIENT,
                              ...
                              subject="Play公社回報, %s, History-New-relationnotice 取得:%s" % (dateString, addCount),
